                  ...
                  Object.keys(cartItems).map((product_id) => {
                    ...
                    <td className="">
                      <div className="flex gap-3">
                        <button onClick={()=>decreaseQuantity(product._id)} className=" w-6 md:w-10 flex items-center justify-center text-2xl shadow-[0_8px_30px_rgb(0,0,0,0.20)] border">
                          -
                        </button>
                        <div className=" px-3 md:px-0 md:w-20 flex items-center justify-center text-2xl shadow-[0_8px_30px_rgb(0,0,0,0.20)] border">
                          {cartItem}
                        </div>
                        <button onClick={()=>increaseQuantity(product._id)} className=" w-6 md:w-10 flex items-center justify-center text-2xl shadow-[0_8px_30px_rgb(0,0,0,0.20)] border">
                          +
                        </button>
                      </div>
                    </td>
                    <td className="text-[#72AEC8] font-bold">${(product.price * cartItem).toFixed(2)}</td>
                    <td><button onClick={()=>deleteItem(product._id)}>X</button></td>
                  </tr>
                    );
                  })
                )}
